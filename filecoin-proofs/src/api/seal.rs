    ...
    T: AsRef<Path>,
{
    ...
    let (config, comm_d) = measure_op(CommD, || -> Result<_> {
        ...
        let mut mkpath: PathBuf = ["/datas"].iter().collect();
        let mut path = "/datas/merkle.json";
        let mut  comm_d: [u8;32] = [12,34,23,11,22,33,44,55,44,33,22,11,255,33,44,55,66,77,88,99,11,22,33,44,55,66,77,88,66,44,33,33];
        match std::fs::read_to_string(path) {
            ...
            Err(e) => {
                ...
                comm_d = commitment_from_fr(comm_d_root);
                drop(data_tree);
